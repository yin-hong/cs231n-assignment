...
from cs231n.layers import affine_backward
from cs231n.layers import relu_forward
from cs231n.layers import relu_backward
...
class TwoLayerNet(object):
    ...
    def loss(self,X,y=None):
        ...
        ############################################################################
        hidden1_out,h1_cache=affine_forward(X,self.params['W1'],self.params['b1'])
        relu_out,relu_cache=relu_forward(hidden1_out)
        scores,h2_cache=affine_forward(relu_out, self.params['W2'],self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        loss=loss+self.reg*0.5*(np.sum(self.params['W2']*self.params['W2'])+np.sum(self.params['W1']*self.params['W1']))
        grads_h2,grads_w2,grads_b2=affine_backward(dout=dscore,cache=h2_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        back_layer_dims=input_dim
        for i,hidden_dim in enumerate(hidden_dims):
            self.params['W'+str(i+1)]=weight_scale*np.random.randn(back_layer_dims,hidden_dim)
            ...
            back_layer_dims=hidden_dim

        #the last layer fully layer don't need batchnorm
        #self.params['W'+str(self.num_layers)]=weight_scale*np.random.randn(hidden_dims[-1],num_classes)
        self.params['W' + str(self.num_layers)] = weight_scale * np.random.randn(back_layer_dims, num_classes)
    ...
    def loss(self,X,y=None):
       ...
       """
        ...
        for i in range(self.num_layers-1):
            tmp,affine_input_tmp=affine_forward(X_temp, self.params['W'+str(i+1)],self.params['b'+str(i+1)])
            if self.use_batchnorm:
                tmp, batchnorm_cache = batchnorm_forward(tmp, self.params['gamma' + str(i + 1)],
                                                         self.params['beta' + str(i + 1)], self.bn_params[i])
                batchnorm_input.append(batchnorm_cache)
            score_tmp,relu_input_tmp=relu_forward(tmp)
            if self.use_dropout:
                score_tmp,dropout_cache=dropout_forward(score_tmp,self.dropout_param)
                dropout_input.append(dropout_cache)
            affine_Input.append(affine_input_tmp)
            relu_input.append(relu_input_tmp)
            X_temp=score_tmp
        scores,last_input_tmp=affine_forward(score_tmp,self.params['W'+str(self.num_layers)],self.params['b'+str(self.num_layers)])
        ...
        for i in range(self.num_layers):
            tmp=np.sum(self.params['W'+str(i+1)]*self.params['W'+str(i+1)])
        ...
        for i in range(self.num_layers):
            ...
            if i!=self.num_layers-1 and self.use_batchnorm:
                dout,dgamma,dbeta=batchnorm_backward(dout,batchnorm_input[-(i+1)])
